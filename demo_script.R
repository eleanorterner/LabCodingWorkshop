...
n_data <- 200 # how many data points to simulate
...
run_linear_model <- function(n_data, # number of data
                             true_m_f,
                             true_b_f, 
                             err_sigma_f
){
  ...
  X <- model.matrix(fit)
  vcov <- vcov(fit)
  out <- sqrt(vcov[2,2]) # standard error of the linear term
  return(out)
}
...
run_linear_model(n_data = 200,
                 true_m_f = true_m, 
                 true_b_f = true_b,
                 err_sigma_f = err_sigma)

# what if I want to do that 1000 times to know what the average se is?
run1000 <- c()
startt <- Sys.time()
for(i in 1:1000){
  run1000[i] <- run_linear_model(n_data = 200,
                   true_m_f = true_m, 
                   true_b_f = true_b,
                   err_sigma_f = err_sigma)
}
fortime <- difftime(Sys.time(), startt)

starttapply <- Sys.time()
apply(as.array(1:1000), 1, function(i){
  out <- run_linear_model(n_data = 200,
                   true_m_f = true_m, 
                   true_b_f = true_b,
                   err_sigma_f = err_sigma)
  return(out)
})
applytime <- difftime(Sys.time(), starttapply)
sapply(1:1000, )
lapply(as.list(1:1000))


# what if I want to do it 1000 times each for different values of n_data?
manyn <- sapply(c(50, 75, 200, 500, 1000), function(n_data_a){
  apply(as.array(1:1000), 1, function(i){
    out <- run_linear_model(n_data = n_data_a,
                            true_m_f = true_m, 
                            true_b_f = true_b,
                            err_sigma_f = err_sigma)
    return(out)
